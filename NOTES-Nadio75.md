...
| Commit | Type | Why |
|---|---|---|
| Add search function to filter team by name | `feat` | Adds new functionality — a new capability the CLI didn't have before |
| Handle missing team.json file gracefully | `fix` | Fixes a failure case (crash/bad behavior) rather than adding new behavior |
| Add usage instructions to README | `docs` | Pure documentation, no code changed |
| Add CI workflow to verify required documentation files exist | `chore` (arguably `ci`) | This is tooling/infrastructure, not app behavior — some Conventional Commits configs add a dedicated `ci` type for exactly this, which this commit would fit even better than `chore` |
| Resolve README conflict between purpose statement and description | doesn't map cleanly | see below |
